	...
	(function () {
		...
		(function () {
			RegexNode = class_create(Literal, {
				...
				createIndexedMatch: function createIndexedMatch(internalMatch) {
					var match = new Match();
				...
				exec_: function exec_(str_, i, opts) {
					var str = str_;
					if (this.cursor != null) {
				...
				createIndexed: function createIndexed(flags) {
				...
				wrappGroups_: function wrappGroups_() {
			...
			function resolveIndexedGroups(match, nativeMatch, nativeIndexerMatch, node, pos) {
				...
				var nextPos = pos;
				for (var el = node.firstChild; el != null; el = el.nextSibling) {
					var next = resolveIndexedGroups(match, nativeMatch, nativeIndexerMatch, el, nextPos);
					if (el.isIncluded !== false) {
						nextPos = next;
					}
				}
			...
			//function resolveIndexedMatchGroups(match, nativeMatch, nativeIndexerMatch, node, pos) {
			//	if (node.groupNum != null && node.groupNum !== 0) {
			//		var shadowMatch = nativeIndexerMatch[node.shadowGroupNum];
			//		var actualMatch = nativeMatch[node.groupNum];
			//		if (actualMatch !== shadowMatch) {
